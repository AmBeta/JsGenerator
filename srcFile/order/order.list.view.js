(function ($, window) {
	...
	var OrderQueryDishHotTableHeaderCfg = [
		...
		{key : "sumAvgPrice", clz : "sort", label : "平均价格", sortkey:"sumAvgPrice"},
		{key : "sumMaster", clz : "sort", label : "销售份数", sortkey:"sumMaster"},
		{key : "sumPrice", clz : "sort", label : "销售总额", sortkey:"sumPrice"}
	];
	...
	var OrderQueryUserTableHeaderCfg = [
		...
		{key : "sumRecord", clz : "sort", label : "订餐次数", sortkey:"sumRecord"},
		{key : "foodAmount", clz : "sort", label : "订餐金额", sortkey:"foodAmount"},
		{key : "unitPrice", clz : "sort", label : "客单价(平均每单消费金额)", sortkey:"unitPrice"},
		{key : "minOrderTime", clz : "date", label : "首次订餐时间"},
		{key : "maxOrderTime", clz : "date", label : "最近订餐时间"}
	];

	var OrderQueryGratuityTableHeaderCfg = [
		{key : "billDate", clz : "date", label : "日期"},
		{key : "shopName", clz : "text", label : "店铺名称"},
		{key : "empName", clz : "text", label : "员工姓名"},
		{key : "orderCount", clz : "sort", label : "次数",sortkey:"orderCount"},
		{key : "orderTotal", clz : "sort", label : "应结金额",sortkey:"orderTotal"},
		{key : "shouldSettlementTotal", clz : "sort", label : "结算金额",sortkey:"shouldSettlementTotal"}
	];
	...
	var QueryOrderGratuityStatisticKeys = 'sumOrderCount,sumOrderTotal,sumShouldSettlementTotal';
	var mapColItemRenderData = function (row, rowIdx, colKey) {
		...
		switch(colKey) {
			case "cityID":
				...
				break;
			case "userName":
			    var clientType= $XP(row, "clientType", '');
				var m = $XP(row, 'userMobile', ''),
					s = Hualala.Common.getGender($XP(row, 'userSex', ''));
				// Fix BUG#7405 : only show '...' when name length exceeds 10 bytes(5 chinese charactors)
				// var _shortName = Hualala.Common.substrByte(v, 10) + '...';
				var _shortName = Hualala.Common.substrByte(v, 10);
				_shortName = (_shortName == v) ? _shortName : (_shortName + '...');
				s = IX.isEmpty(s) ? '' : '(' + $XP(s, 'label', '') + ')';
				m = IX.isEmpty(m) ? '' : '(' + m + ')';
				r.value = $XP(row, 'userID', '');
				if(!_.contains(["50","51","55","56"],clientType)){
			...
			case "sumMaster":
				r.value = v;
				r.text = v.replace(/\.[0]*$/, '') + "&nbsp;" + $XP(row, 'foodUnit', ''); // “份数”小数全为0时不显示
				break;
			case "sumAvgPrice":
				r.value = v || 0;
	 ...
	 * @return {[type]}
	 */
	Hualala.Order.mapQueryGratuityRenderData = function (records) {
		var self = this;
		var clz = "col-md-12",
			tblClz = "table-bordered table-striped table-hover ix-data-report",
			tblHeaders = OrderQueryGratuityTableHeaderCfg,			
			statisticData = self.model.getStatisticData();
		var mapColsRenderData = function (row, idx) {
			var colKeys = _.map(tblHeaders, function (el) {
				return _.pick(el, 'key', 'clz');
		...
		var statisticKeys = QueryOrderGratuityStatisticKeys;
		var ftCols = _.map(statisticKeys.split(','), function (k) {
			var v = $XP(statisticData, k, 0), rowspan = 1, clz = '', 
				colspan = k == "needPayAmount" ? 3 : 1;
			if(k=="sumOrderCount"){
	...
	OrderQueryResultView.proto({
		...
		bindEvent : function () {
			var self = this,
				pagename = Hualala.PageRoute.getPageContextByPath().name;
			self.model.hasPager && self.$pager.on('page', function (e, pageNo) {
				var params = self.model.getPagerParams();
					params['pageNo'] = pageNo;
				if(pagename=="orderQueryGratuity"){
					var $ascsort = $('.ix-data-report th.sort.asc'),
						$descsort = $('.ix-data-report th.sort.desc')
					if($ascsort.length!=0){
						var sortkey = $ascsort.attr("sort-key");
						switch(sortkey){
			            	case "orderCount":
			            		params.orderBy =1;
			            	break;
				        ...
				        params.sort="asc";
					    self.model.emit("rewardOrderSort",{
			            	params : params,
			            	cbFn : function(){
								self.emit('render');
								self.$container.find('thead [sort-key='+sortkey+']').addClass(params.sort);
			            	}
			            });	
					}else if($descsort.length!=0){
						var sortkey = $ascsort.attr("sort-key");
						switch(sortkey){
			            	case "orderCount":
			            		params.orderBy =1;
			...
			self.$resultBox.on('click', 'a.query-order-detail', function (e) {
			...
			}).on('click', '.ix-data-report .sort', function (e){
				var $allsort = self.$resultBox.find('.ix-data-report .sort');
	            var $this = $(this), sortClass = '';
	            	$allsort.not(this).removeClass('asc desc');
	            if($this.hasClass('asc'))
	           	...
	           	var params = self.model.getPagerParams();
	            params.sort = sortClass;
	            var callKey = $this.attr('sort-key');
	            if(pagename=="orderQueryGratuity"||pagename=="orderDishesHot"){
		            switch(callKey){
		            	case "orderCount":
		            	case "sumAvgPrice":
		            		params.orderBy =1;
		          	...
		          	var api = pagename=="orderQueryGratuity"?"rewardOrderSort":"dishesHotSort";
		            self.model.emit(api,{
		            	params : params,
		            	cbFn : function(){
							self.emit('render');
							self.$container.find('thead [sort-key='+callKey+']').addClass(sortClass);
		            	}
		            });		            	
	            }else{
		            switch(callKey){
		            	case "sumRecord":
				            self.model.emit('sumRecordSort',{
				            	params : params,
				            	cbFn : function(){
									self.emit('render');
									self.$container.find('thead [sort-key='+callKey+']').addClass(sortClass);
		            	...
		            	break;
		            	case "foodAmount":
		            		self.model.emit('foodAmountSort',{
				            	params : params,
				            	cbFn : function(){
									self.emit('render');
									self.$container.find('thead [sort-key='+callKey+']').addClass(sortClass);
	...
	Hualala.Order.QueryOrderGratuityStatisticKeys=QueryOrderGratuityStatisticKeys;
})(jQuery, window);