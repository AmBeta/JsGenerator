		...
		bindEvent : function () {
			...
			self.$footer.on('click', '.btn-ok', function (e) {
				...
				if(self.parentView.model){
					...
					var shopIDs = _.pluck($XP(chosenData, 'shops'), 'shopID');
					self.trigger.find('.choose-shop-val').attr("data-shop",shopIDs);
					self.trigger.find('.choose-shop-val').html(shopNames.length == 0 ? '不限店铺' : shopNames.join(';'));
				}
				self.modal.hide();
			});
