		...
		switch(colKey) {
			...
			case "giftType":
				...
				var card = giftCardTpl({
					...
					labelShort : giftLabelShort,
					labelFull:giftLabelFull,
					val : giftType.value == 80 ? '':giftValue,
			...
			case "eventName":
				...
				if(eventWay==50||eventWay==53||eventWay==51){
					...
					// STORY #2002 【dianpu】营销短信增加审核标记
					if(smsGate==2){
						r.text = v+'<br/><br/>'+'活动状态：'+statusLabel+'<br/>'+'结算状态：不需要结算';
					}else{
						r.text = v+'<br/><br/>'+'活动状态：'+statusLabel+'<br/>'+'结算状态：'+settleStatusLabel;
					}					
				}else{
					r.value = v;
					r.text = v;
			...
			case "isActive":
				...
				r.value = v;
				// 群发短信/群发礼品
				if(eventWay==50||eventWay==53){
					// 待开始、审核中   开关可以开启或关闭
					if(status==1||status==5){
						r.text = '<input type="checkbox" name="switch_event" data-eventWay="' + eventWay + '" data-sms="' + smsGate + '" data-event-id="' + eventID + '" ' 
						+ (v != 0 ? 'checked ' : '') + ' data-key="' + $XP(row, '__id__', '') + '" ' + ' />';
					} else{
						r.text = '<input type="checkbox" disabled  name="switch_event" data-eventWay="' + eventWay + '" data-sms="' + smsGate + '" data-event-id="' + eventID + '" ' 
						+ (v != 0 ? 'checked ' : '') + ' data-key="' + $XP(row, '__id__', '') + '" ' + ' />';
					}
				// 生日赠送
				}else if(eventWay==51){
			...
			switch(act) {
				...
				// TODO edit event config set
                    var wizardStepChangeMap = {
	                        ...
	                        '53': Hualala.MCM.bulkGiftsEventStepChange,
	                        '52': Hualala.MCM.opencardGiftEventStepChange
                        },
	...
	var mapEventDetailRenderData = function (model) {
		...
		var mapBaseInfoData = function () {
			...
			_.each(keys, function (k) {
				switch (k) {
					case 'cardLevelLabel':
                        var cardLevelID = model.get('cardLevelID');
						ret[k] = _.find(cardLevels, function (el) {
							return $XP(el, 'value') == cardLevelID;
						});
						ret[k] = cardLevelID == -1 ? '不限' : $XP(ret[k], 'label');
						break;
					case 'deductPoints':
					case 'sendPoints':
			...
			var smsSendTime = model.get('startTime') || 0,
				startTimeLabel = '';
			if(eventWay == 50 ||eventWay == 53|| smsGate == 1) {
				...
				ret = IX.inherit(ret, {
					'cardGroupName' : model.get('cardGroupName') || 0,
					'cardCount' : parseInt(model.get('cardCount') || 0)
				});
			}
			...
			return IX.inherit(ret, {
				hiddenGroup: eventWay == 50 ||eventWay == 53?'hidden':'',
				disGroup: eventWay == 50 ||eventWay == 53?'':'hidden',
				customerRange: '会员等级最低要求',
				infoLabelClz : 'col-xs-3 col-sm-3',
				infoTextClz : 'col-xs-8 col-sm-8',
                smsStartLabel: eventWay == 51 ? '提前赠送天数' : '开始发送时间',
				startTimeLabel: startTimeLabel,
                hiddenShop: eventWay == 50? '' : 'hidden',
                lastTransTime: lastTransTimeFilterLabel + (lastTransTimeFilter == 0 ? '' : ' ' + lastTransTimeFormat),
                smsAlert: (eventWay == 50 ||(eventWay == 53&&smsGate==1))? '短信活动一旦开始不得停止' : ''
			});
		};
		var mapGiftsData = function () {
			...
			return {
                ...
                isSingleCount: _.contains(['40', '41', '51','53','52'], eventWay),
                hideSurplusCount: (eventWay == 51||eventWay == 53||eventWay == 52) ? 'hidden' : '',
				hideEGfitValidUntilDayCount : '',
				hiddenGift: (eventWay==50||eventWay==22) ? 'hidden' : '',
				hiddenSms: (eventWay==50 || eventWay == 51||eventWay == 53) ? '' :'hidden',
                hiddenCustomerRange: (eventWay == 51 ||eventWay == 52||eventWay==53)? 'hidden' : '',
				hiddenEvtRules : (eventWay == 50 || eventWay == 51||eventWay == 53||eventWay == 52) ? 'hidden' :'',
	...
	var initEventDetailModal = function ($btn) {
		...
		var loadCardLevels = function () {
			itemModel.emit('loadCardLevelIDs', {
				successFn : function (res) {
					var des = {},
						renderData = mapEventDetailRenderData.call(self, itemModel),
						eventWay = itemModel.get('eventWay'),
						rewardOnly = itemModel.get('rewardOnly');
						cardLevelID = itemModel.get('cardLevelID');

                    //当活动会员范围为不限时，隐藏积分信息，只显示打赏标记
                    if ( eventWay == 20 || eventWay == 21) {
                    	des.hideRewardOnly = '';
                    	des.rewardOnly = rewardOnly == '0'?'否':'是';
                    	if( cardLevelID == -1){
                    		des.hideDeductPoints = 'hidden';
                    		des.hideSendPoints = 'hidden';
                    	}else{
                    		des.hideDeductPoints = '';
                    		des.hideSendPoints = '';
                    	}           	
                    }else{
                    	des.hideRewardOnly = 'hidden';
                    	des.hideDeductPoints = '';
                    	des.hideSendPoints = '';
                    }
                    renderData =  IX.inherit(renderData,des);
                    //初始化模板
                    $detail = $(layoutTpl(renderData));
                    modal._.body.append($detail);
					modal.show();
                    //打赏仅限于活动20.21
                    /*if(eventWay != 20 && eventWay != 21) {
                    	$(".form-group:contains('仅用于打赏活动')").hide();
                    }*/

                    if(eventWay == 50 || eventWay == 51) {
                        var $smsTemplate = modal._.body.find('[name="sms_template"]');
                        HMCM.getSMSSign({
                            successFn: function(signName) {
                                $smsTemplate.text($smsTemplate.text() + signName);
                            }
                        });
                    }
				},
		...
		$checkbox.each(function () {
			...
			$el.bootstrapSwitch({
				...
				var eventID = $el.attr('data-event-id'),
					...
					eventWay = $el.attr('data-eventWay'),
					smsGate = $el.attr('data-sms');
					...
					okFn : function () {
						self.emit('switchEvent', {
							...
							faildFn : function (res) {
								...
								if((eventWay==50||eventWay==51||eventWay==53)&&res.resultcode == 'BP10001'){
									toptip({
										msg : "因您的账户余额为负值，活动不能被启用，请至结算下进行账户充值后启用活动。",
										type : 'danger'
									});	
								}else{
									toptip({
